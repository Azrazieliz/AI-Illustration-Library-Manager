...
from engine.automation.automation_models import AutomationCheckpoint, AutomationOperationResult
from engine.automation.automation_service import AutomationService
from engine.pipeline import PipelineJob, QueueManager, QueueType


class AutomationWorker:
    """Background worker consuming automation-stage jobs from SEARCH queue."""
    ...
    def __init__(
        ...
        *,
        ...
        self.service = service or AutomationService(queue_manager=self.queue_manager)
        self._checkpoint = AutomationCheckpoint()
    ...
    def start(self) -> None:
        ...
        self._thread = Thread(target=self._run, daemon=True, name="automation-worker")
        self._thread.start()
    ...
    def process_jobs(self, jobs: list[PipelineJob]) -> list[AutomationOperationResult]:
        results: list[AutomationOperationResult] = []
        for job in jobs:
            if self._should_skip(job):
                continue
            result = self.service.process_automation_job(job, checkpoint=self._checkpoint)
    ...
    def _run(self) -> None:
        ...
        while not self._stop_event.is_set():
            ...
            if self._should_skip(job):
                continue
            if (job.metadata or {}).get("stage") != "automation":
                continue
            self.service.process_automation_job(job, checkpoint=self._checkpoint)
