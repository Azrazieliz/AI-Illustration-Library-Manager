...
class OrganizerStatistics:
    """Thread-safe counters and timing for organizer operations."""
    ...
    duplicate_moves_prevented: int = 0
    rolled_back: int = 0
    failed: int = 0
    dry_run_count: int = 0
    ...
    def increment_already_organized(self) -> None:
        with self._lock:
            self.already_organized += 1

    def increment_duplicate_moves_prevented(self) -> None:
        with self._lock:
            self.duplicate_moves_prevented += 1
